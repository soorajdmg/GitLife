...
export default function GraphToolbar({ mode, onModeChange, loadBearingOnly, onLoadBearingToggle, onTidy, searchQuery, onSearchChange, onSearchJump, isDark }) {
  ...
  const divColor = isDark ? 'oklch(28% 0.015 260)' : 'oklch(93% 0.005 260)';
  ...
  if (IS_MOBILE) {
    return (
      <div style={{
        ...
        boxShadow: '0 4px 16px oklch(25% 0.05 260 / 0.14)',
        border, fontFamily: "'Plus Jakarta Sans', sans-serif",
        display: 'flex', flexDirection: 'column',
      }}>
        {/* Row 1: mode + load-bearing + tidy + collapse */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '6px 8px' }}>
          ...
          <VDivider isDark={isDark} />

          {/* Load-bearing — icon-only on mobile to save space */}
          <button
            style={pill(loadBearingOnly, isDark, { padding: '5px 9px' })}
            onClick={onLoadBearingToggle}
            title="Show only load-bearing decisions"
          >
            ◈
          </button>
          ...
          {/* Tidy */}
          <button style={pill(false, isDark)} onClick={onTidy} title="Tidy layout">
            ✦ Tidy
          </button>

          {/* Collapse — pushed to the right */}
          <button
            onClick={() => setCollapsed(true)}
            style={{ marginLeft: 'auto', background: 'none', border: 'none', cursor: 'pointer', padding: '4px 4px', display: 'flex', alignItems: 'center', color: isDark ? 'oklch(50% 0.01 260)' : 'oklch(60% 0.01 260)', flexShrink: 0 }}
          >
            <svg width="11" height="11" viewBox="0 0 12 12" fill="none">
        ...
        <div style={{ borderTop: `1px solid ${divColor}`, padding: '6px 8px' }}>
  ...
  return (
    <div style={{
      ...
      {/* Search row */}
      <div style={{ borderTop: `1px solid ${divColor}`, padding: '6px 10px' }}>
        <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
          <span style={{ position: 'absolute', left: 8, fontSize: 11, color: 'oklch(62% 0.01 260)', pointerEvents: 'none', userSelect: 'none' }}>⌕</span>
          <input
            ref={inputRef}
